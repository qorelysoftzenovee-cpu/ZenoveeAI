  ...
  const content = (
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur sm:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          {resolvedTool.inputs.map((input) => (
            <div key={input.id}>
              <label htmlFor={input.id} className="mb-3 block text-sm font-medium text-white/80">
                {input.label}
              </label>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-white px-5 py-4 text-sm font-semibold text-slate-950 transition hover:bg-cyan-100 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isLoading ? (
              <>
              ...
              `Run Tool - Deducts ${resolvedTool.cost} Credits`
            )}
          </button>
        </form>
      </section>
      ...
      <section className="rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur sm:p-8">
        ...
        {isLoading ? (
          <div className="mt-8 flex min-h-[220px] flex-col items-center justify-center rounded-[1.75rem] border border-dashed border-cyan-300/20 bg-[#08101f] text-center">
          ...
          <article className="mt-8 rounded-[1.75rem] border border-white/10 bg-[#08101f] p-6 shadow-lg shadow-cyan-950/10">
