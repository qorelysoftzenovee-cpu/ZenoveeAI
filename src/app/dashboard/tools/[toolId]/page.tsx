...
export default function ToolWorkspacePage() {
  ...
  return (
    <div className="text-slate-800 animate-fade-in-up font-sans">
      ...
      {/* Responsive Horizontal Banner Advertisement Slot */}
      <div className="h-24 w-full bg-slate-50 border border-slate-200/50 rounded-xl my-6 flex items-center justify-center text-xs text-slate-400 font-mono uppercase tracking-wider">
        ADVERTISEMENT
      </div>
      ...
      <div className="grid min-h-[calc(100vh-22rem)] grid-cols-1 gap-6 lg:grid-cols-2 items-stretch">
          ...
          <div className="flex-1 space-y-5 p-6 overflow-y-auto">
            <div className="mb-2">
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[10px] font-bold uppercase transition-all duration-200 cursor-pointer ${
                  presetLoaded
              ...
              <div key={inputField.id} className="space-y-2">
                ...
                {inputField.type === "file" ? (
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[140px] rounded-xl border border-slate-200 bg-slate-50/50 p-3.5 text-xs text-slate-800 placeholder:text-slate-400 transition-all duration-200 resize-none focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400/20 focus:border-indigo-400"
                  />
                  ...
                  <div className="relative">
                    <select
                      ...
                      value={inputs[inputField.id] || ""}
                      onChange={(e) => handleInputChange(inputField.id, e.target.value)}
                      className="w-full appearance-none rounded-xl border border-slate-200 bg-slate-50/50 p-3.5 pr-10 text-xs text-slate-800 transition-all duration-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400/20 focus:border-indigo-400"
                    >
                  ...
                  <input
                    ...
                    value={inputs[inputField.id] || ""}
                    onChange={(e) => handleInputChange(inputField.id, e.target.value)}
                    className="w-full rounded-xl border border-slate-200 bg-slate-50/50 p-3.5 text-xs text-slate-800 placeholder:text-slate-400 transition-all duration-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400/20 focus:border-indigo-400"
                  />
                )}
              </div>
            ))}
          ...
          <div className="border-t border-slate-100 bg-slate-50/40 p-5 flex items-center justify-between">
            <div className="flex items-center gap-3 flex-1">
              <button
                type="submit"
                disabled={loading}
                className="group relative flex-1 overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-indigo-650 to-blue-600 py-3.5 px-5 text-xs font-bold uppercase tracking-wider text-white shadow-md transition-all duration-200 hover:shadow-indigo-500/20 hover:scale-[1.005] cursor-pointer"
              >
              ...
              {(output || Object.keys(inputs).length > 0 || selectedFile !== null) && (
                <button
                  type="button"
                  onClick={handleReset}
                  className="flex items-center gap-1 text-xs font-semibold text-slate-400 hover:text-slate-700 transition-colors cursor-pointer px-2"
                >
        ...
        <div className="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          {/* Card Terminal Body */}
          <div className="flex-1 overflow-y-auto bg-[#F8FAFC] relative text-slate-800 font-sans text-sm leading-relaxed p-6 selection:bg-indigo-100 selection:text-indigo-900">
            {loading && (
          ...
          {output && !loading && (
            <div className="flex items-center justify-end gap-2 border-t border-slate-200 bg-white px-5 py-3 shrink-0 flex-wrap">
              <button
                type="button"
                onClick={handleCopy}
                className="inline-flex items-center gap-1.5 rounded-lg bg-white border border-slate-200 px-3.5 py-2 text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all cursor-pointer shadow-sm"
              >
              ...
              <button
                type="button"
                onClick={handleShare}
                className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 border border-indigo-200 px-3.5 py-2 text-xs font-semibold text-indigo-600 hover:bg-indigo-100 transition-all cursor-pointer shadow-sm"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-white border border-slate-200 px-3.5 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-all cursor-pointer shadow-sm"
              >
