  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        {/* HERO SECTION */}
        <section className="relative overflow-hidden pt-24 md:pt-32 pb-16 md:pb-16 md:pb-24">
          ...
          <div className="particle-field">
            {Array.from({ length: 20 }).map((_, i) => {
              // Deterministic pseudo-random values based on index to avoid hydration mismatch and purity lint errors
              const pseudoRand = (seed: number) => ((seed * 9301 + 49297) % 233280) / 233280;
              return (
                <div 
                  key={i} 
                  className="particle" 
                  style={{ 
                    left: `${pseudoRand(i + 1) * 100}%`, 
                    top: `${pseudoRand(i + 2) * 100}%`,
                    animationDelay: `${pseudoRand(i + 3) * 5}s`,
                    animationDuration: `${3 + pseudoRand(i + 4) * 4}s`
                  }}
                />
              )
            })}
          ...
          <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
            ...
            <h1 className="text-5xl sm:text-7xl font-extrabold tracking-tight leading-[1.1] anim-fade-up delay-100 text-slate-900">
              The Ultimate Free Tool Suite<br />
              <span className="gradient-text-animated mt-2 block">For Developers & Marketers.</span>
            </h1>

            <p className="mt-6 text-lg sm:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed anim-fade-up delay-200">
        ...
        {/* SEARCH SECTION */}
        <section className="max-w-2xl mx-auto px-6 -mt-8 mb-10 md:mb-20 relative z-20 anim-fade-up delay-400">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 w-5 h-5 transition-colors group-focus-within:text-violet-400" />
            <input
              type="text"
            ...
            {searchQuery && (
              <button 
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-zinc-300 transition-colors p-1"
        ...
        <section className="max-w-7xl mx-auto px-6 mb-16 md:mb-32 relative z-10">
          ...
          {filteredTools.length > 0 ? (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4 stagger">
              {filteredTools.map((tool) => (
                <Link href={`/dashboard/tools/${tool.id}`} key={tool.id} className="tool-card group anim-fade-up block">
                  ...
                  <h3 className="text-base font-semibold text-slate-900 group-hover:text-violet-300 transition-colors">
                    {tool.title}
                  </h3>
            ...
            <div className="text-center py-20 glass-card rounded-3xl">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 mb-4 ring-1 ring-slate-200">
                <Search className="w-8 h-8 text-slate-500" />
              </div>
              <h3 className="text-xl font-semibold text-slate-900">No tools found</h3>
              <p className="text-slate-600 mt-2">Try adjusting your search terms or browse all categories.</p>
              <button 
                onClick={() => setSearchQuery('')}
                className="mt-6 px-6 py-2 bg-zinc-800 text-slate-900 rounded-full text-sm font-medium hover:bg-slate-700 transition-colors ring-1 ring-slate-200"
              >
                Clear Search
              </button>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 py-16 md:py-24 relative">
          ...
          <div className="text-center mb-16 pt-8">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-900">Instant results. Zero friction.</h2>
          </div>
          ...
          <div className="grid gap-8 md:grid-cols-3">
            <div className="glass-card rounded-2xl p-8 text-center relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Find</h3>
                <p className="text-slate-600">Search instantly through {toolsConfig.length}+ specialized utilities.</p>
              </div>
            </div>
            ...
            <div className="glass-card rounded-2xl p-8 text-center relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3">Execute</h3>
                <p className="text-slate-600">Run locally in your browser. No server delays, total privacy.</p>
              </div>
            </div>
            ...
            <div className="glass-card rounded-2xl p-8 text-center relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-slate-600">Copy output, download files, and get back to shipping.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 pb-16 md:pb-24">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold text-slate-900">Built for your stack</h2>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 stagger">
            {categories.map((category) => (
              <Link 
                ...
                key={category.name} 
                className="glass-card rounded-xl p-5 text-center anim-fade-up transition-all group"
              >
                <h3 className="text-sm font-semibold text-slate-900 group-hover:text-cyan-400 transition-colors">{category.name}</h3>
                <span className="block text-xs text-slate-500 mt-1">{category.count} tools</span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 pb-32">
          <div className="glass-card rounded-3xl p-12 relative overflow-hidden border border-slate-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center relative z-10">
              <div>
                ...
                <div className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Free Tools</div>
              </div>
              <div>
                <div className="gradient-text text-4xl md:text-5xl font-extrabold mb-2">{categories.length}</div>
                <div className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Categories</div>
              </div>
              <div>
                <div className="gradient-text text-4xl md:text-5xl font-extrabold mb-2">0</div>
                <div className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Server Calls</div>
              </div>
              <div>
                <div className="gradient-text text-4xl md:text-5xl font-extrabold mb-2">100%</div>
                <div className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Private</div>
              </div>
            </div>
          </div>
        </section>
        ...
        {/* CTA SECTION */}
        <section className="max-w-7xl mx-auto px-6 pb-32">
          <div className="rounded-3xl bg-gradient-to-br from-violet-600 to-indigo-800 p-12 md:p-20 text-center relative overflow-hidden shadow-2xl border border-slate-200">
            ...
            <div className="absolute top-0 right-0 w-96 h-96 bg-violet-900/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3"></div>
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-violet-900/10 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3"></div>
            
            <div className="relative z-10">
              <h2 className="text-3xl sm:text-5xl font-bold text-slate-900 tracking-tight mb-6">Ready to upgrade your workflow?</h2>
              <p className="text-lg text-slate-900/70 max-w-2xl mx-auto mb-10">
